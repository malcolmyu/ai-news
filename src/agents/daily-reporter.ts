...
export class DailyReporter {
  ...
  async generateDailyReport(date?: Date, options?: {
    ...
    try {
      ...
      const validation = harness.checkFile(reportFile);
      if (!validation.valid) {
        this.logger.warn(`Harness quality gate failed: ${validation.errors.join(', ')}`);
      } else {
        this.logger.log('✅ Harness quality gate passed');
      }

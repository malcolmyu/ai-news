...
docs/
  ...
  daily/                      # 日报 HTML + Daily Source JSON + 媒体资源 + 归档
  research/                   # 调研报告 + 截图 + 归档
scripts/
  ...
  render_daily.py             # Daily Source JSON → 日报 HTML
  ingest_daily_html.py        # 历史 HTML → Daily Source JSON
...
3. 每日日报优先写入 `docs/daily/data/YYYY-MM-DD.json`，再由 `npm run site:render-daily -- --date YYYY-MM-DD` 渲染 HTML
